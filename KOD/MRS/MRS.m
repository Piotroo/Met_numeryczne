%--skrypt do symulacji obliczenia pola potencjalu metoda roznicskonczonych
%-- rozwiazanierownania Laplace'a
clear; clc; format shortg;
Nx = 30; Ny = 30; % rozmiar siatki
Lx = 1; Ly = 1; % wielkość (np. temperatura)
x = linspace(0, Lx, Nx); % podział wielkości na ilośc kroków (rozmiar na 
rowne cześci) na x 
y = linspace(0, Ly, Ny); % podział wielkości na ilośc kroków (rozmiar na 
rowne czesci) na y
dx = x(2) - x(1); dy = y(2) - y(1); % policzenie wielkości różnic pomiędzy 
krokami w osi x oraz y
...
n = i+(j-1)*Nx; %--indexij na liniowy
 ...
 B(n, 1) = 0; %--tu nie potrzebne w przypPoisonasourceterms
end
end
%% Warunki brzegowe
%lewa krawędź phi=y
