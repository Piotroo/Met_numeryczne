...
R = [R1, 0; ... % mała macierz R
 0, R2 ];
% Duże macierze
A = [L, zeros(2); zeros(2), eye(2)]; % macierz A złożona z macierzy L oraz 
zerowych i jednostkowych
B = [ R, C; -eye(2), zeros(2)]; %macierz B złożona z macierzy R oraz C 
Q = [u;0]; % Napięcie przyłożone do układu
U = [Q; zeros(2,1)]; % wektor siły U
%--definicja warunkowpoczatkowychY0 = zeros(4,1);
%-- alokacja pamieci--
wyn = zeros(4,max(size(t))); wyn(:,1) = Y0;
%% -- rozwiazanie met Eulera - wykazuje niestabilnosc
for k = 1:1:max(size(t))-1
wyn(:,k+1) = wyn(:,k) + A\(U - B*wyn(:,k))*dt;
end
figure(1)
plot(t, wyn(1:2,:), t, (wyn(1,:) - wyn(2,:))); grid on;
legend('I1', 'I2', 'I3')
xlabel('czas'); ylabel('Prąd [A]');
figure(2)
plot(t, wyn(3:4,:)); grid on;
legend('C2', 'C1'); 
xlabel('czas'); ylabel('Ładunek [Culomb]');